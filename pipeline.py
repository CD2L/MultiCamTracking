...
def main(opt):
    ...
    ref_img = cv.resize(ref_img, (224,224))
    ...
    print(ref_preprocessed.shape)
    ref_tensor = model(ref_preprocessed)
    ...
    for  batch_idx, img_batch in enumerate(imgs):
        ...
        img_batch_tensors = model(img_batch_preprocessed)

        distance_res = []
        for img_tensor in img_batch_tensors:
            distance_res.append(distance(ref_tensor,img_tensor, type='cosine').item())
